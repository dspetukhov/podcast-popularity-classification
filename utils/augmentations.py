...
def speed_shift(waveform, sample_rate):
    ...
    """
    sp = T.SpeedPerturbation(
        orig_freq=sample_rate,
        factors=[random.choice([0.9, 1.1])]
    )
    _waveform, _ = sp(waveform)
    return _waveform


def clipping(waveform, clip_length=5_000_000):
    """Taking a random fixed length segment of the waveform."""
    offset = random.randint(0, waveform.shape[0] - clip_length)
    return waveform[offset:(offset+clip_length)]


def masking(spectrogram, frequency, time):
    """Add frequency or time masking."""
    if frequency:
